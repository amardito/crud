...
const find =  async (objCon, param = {}, field = {}, sort = {}) => {
...
const findOne = async (objCon, param = {}, field={}) => {
  const {conn, client} = await con(objCon);
...
const findRange = async (objCon, start = 0, count = 1,param = {}, field={}, sort = {}) => {
  const {conn, client} = await con(objCon);
...
const findPage = async (objCon, page = 1,count = 10,param = {}, field = {}, sort = {}) => {  
  let skp = count * (page-1);
  let res = await findRange(objCon, skp, count, param, field, sort);  
  return res;
}

const getCount = async (objCon, param = {}) => {  
  const {conn, client} = await con(objCon);
